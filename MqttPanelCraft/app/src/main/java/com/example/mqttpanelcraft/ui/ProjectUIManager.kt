...
/**
 * Manages the UI elements of ProjectViewActivity to reduce bloat. Handles Toolbar, BottomSheet,
 * FAB, System Bars, and Layout adjustments.
 */
class ProjectUIManager(
    ...
    fun setDeleteZoneState(state: CanvasInteractionManager.DeleteState) {
        val header = root.findViewById<View>(R.id.bottomSheetHeader) ?: return
        val handle = root.findViewById<View>(R.id.ivHeaderHandle)
        val trash = root.findViewById<ImageView>(R.id.ivHeaderTrash)
        val text = root.findViewById<View>(R.id.tvHeaderDelete) // Unused now

        when (state) {
            CanvasInteractionManager.DeleteState.NONE -> {
                header.background = null
                handle?.visibility = View.VISIBLE
                trash?.visibility = View.GONE
                trash?.scaleX = 1.0f
                trash?.scaleY = 1.0f
                trash?.setColorFilter(Color.WHITE) // Default
                text?.visibility = View.GONE
            }
            CanvasInteractionManager.DeleteState.WARNING -> {
                header.setBackgroundResource(R.drawable.bg_delete_warning)
                handle?.visibility = View.GONE
                trash?.visibility = View.VISIBLE
                trash?.scaleX = 1.0f
                trash?.scaleY = 1.0f
                trash?.setColorFilter(Color.WHITE) // White icon
                text?.visibility = View.GONE
            }
            CanvasInteractionManager.DeleteState.ACTIVE -> {
                header.setBackgroundResource(R.drawable.bg_delete_gradient)
                handle?.visibility = View.GONE
                trash?.visibility = View.VISIBLE
                trash?.scaleX = 1.2f // Pop effect
                trash?.scaleY = 1.2f
                trash?.setColorFilter(Color.WHITE) // White icon
                text?.visibility = View.GONE
            }
    ...
    fun setupToolbar() {
        ...
        // Menu/Hamburger Icon
        val menuIcon =
                ContextCompat.getDrawable(activity, android.R.drawable.ic_menu_sort_by_size)
                        ?.mutate()
        ...
        // Grid Button
        root.findViewById<View>(R.id.btnGrid).setOnClickListener { viewModel.toggleGrid() }
    ...
    fun updateSystemBars() {
        ...
        root.findViewById<CoordinatorLayout>(R.id.rootCoordinator)
    ...
    fun updateModeUI(isEditMode: Boolean, selectedId: Int?) {
        ...
        if (isEditMode) {
            // EDIT MODE
            // Fix: Return TRUE to consume event and block NestedScrollView from scrolling on empty
            // space
            ...
            activity.supportActionBar?.setDisplayHomeAsUpEnabled(true)
            val addIcon =
                    ContextCompat.getDrawable(activity, R.drawable.ic_action_add_large)?.mutate()
            ...
            activity.supportActionBar?.setDisplayHomeAsUpEnabled(true)
            val backArrow =
                    ContextCompat.getDrawable(activity, R.drawable.ic_action_back_large)?.mutate()
    ...
    fun updateCanvasOcclusion(compMaxY: Float = 0f) {
        ...
        if (actuallyEditMode) {
            ...
            val selectedId =
                    (activity as? com.example.mqttpanelcraft.ProjectViewActivity)
