...
object ArduinoCodeGenerator {
        ...
        try {
            ...
            project.components.forEach { comp ->
                ...
                if (templateKey.isNotEmpty() && compTemplates.has(templateKey)) {
                    ...
                    // Append to Mapping
                    sbMapping.append(
                            " ${comp.label} | ${comp.type} | $relTopicBase | $varNameFinal\n"
                    )

                    // Helper to replace all common placeholders
                    fun processPlaceholders(original: String): String {
                        var out =
                                original.replace("{{INDEX}}", idx.toString())
                                        .replace("{{LABEL}}", comp.label)
                                        .replace("{{REL_TOPIC_SET}}", relTopicSet)
                                        .replace("{{REL_TOPIC_VAL}}", relTopicVal)
                                        .replace("{{TYPE_KEY}}", indexKey)

                        // Inject Properties: {{PROP_NAME}} -> value
                        comp.props.forEach { (k, v) ->
                            out = out.replace("{{PROP_${k.uppercase()}}}", v)
                        }
                        return out
                    }
                    ...
                    // --- Setup (Subscribe) ---
                    val setupSub = processPlaceholders(tmpl.optString("setup_sub", ""))
                    if (setupSub.isNotEmpty()) sbSetup.append("  ").append(setupSub).append("\n")

                    // --- Loop Logic (Publish) ---
                    val loopLogic = processPlaceholders(tmpl.optString("loop_logic", ""))
                    if (loopLogic.isNotEmpty()) sbLoop.append(loopLogic).append("\n")

                    // --- Receiver Logic ---
                    val recvLogic = processPlaceholders(tmpl.optString("receiver_logic", ""))
            ...
            header =
                    header.replace("{{BROKER}}", project.broker)
    ...
    private fun generateForWebView(context: Context, project: Project): String {
        try {
            ...
            // B. Find "mqtt.subscribe(VAR_NAME)" OR "mqtt.subscribe('LITERAL')" (App Input ->
            // Arduino Output)
            // C. Find "mqtt.publish(VAR_NAME)" OR "mqtt.publish('LITERAL')" (App Output -> Arduino
            // Input)
            ...
            if (appPubs.isEmpty()) {
                ...
                sb.append(
                        "  // If the App sends commands, ensure it uses mqtt.publish('topic', ...)\n"
                )
            }
            ...
            if (appSubs.isEmpty()) {
                ...
                appSubs.forEach { relPath ->
                    ...
                    sb.append(
                            "  // mqttpanel_pub(String(mqtt_topic_head) + \"$cleanPath\", \"Hello App\");\n"
                    )
                }
                sb.append("  // -------------------------------------\n")
            }
