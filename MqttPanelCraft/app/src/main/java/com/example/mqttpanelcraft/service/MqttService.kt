...
class MqttService : Service() {
    ...
    // Track active connection params to avoid redundant resets
    ...
    private fun connect(
        ...
        // v44.4: Check if already connected to the same destination
        if (mqttClient != null &&
            ...
            MqttRepository.setStatus(MqttStatus.CONNECTED)
            return
        }

        // Cancel previous pending connection job if any
        ...
        connectionJob =
                serviceScope.launch {
                    ...
                    try {
                        ...
                        // Disconnect existing IF it's a different broker
                        ...
                        mqttClient!!.setCallback(
                                object : MqttCallbackExtended {
                                    ...
                                    override fun connectComplete(
                                        ...
                                        // If we need to re-subscribe after auto-reconnect, we can
                                        // do it
                                        // here.
                                        // But main activity usually handles logic. Ideally we
                                        // should track
                                        // subscriptions.
                                        // For now, relies on Activity sending SUBSCRIBE again or
                                        // CleanSession=false persistence.
                                    }
                                }
                        )
                        ...
                        while (!connected &&
                                ...
                                isActive) { // v44: Fix isActive usage
                            attempts++
                            try {
                                MqttRepository.addLog(
