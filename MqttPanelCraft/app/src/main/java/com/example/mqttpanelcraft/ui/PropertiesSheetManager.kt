...
class PropertiesSheetManager(
    ...
    private val btnPropLabelVisibility: ImageView? =
            propertyContainer.findViewById(R.id.btnPropLabelVisibility)

    // Color Picker UI
    // private val vPropColorPreview: TextView? =
    // propertyContainer.findViewById(R.id.vPropColorPreview)
    ...
    private val etPropGenericPayload: EditText? =
            propertyContainer.findViewById(R.id.etPropGenericPayload)

    private val btnSaveProps: Button? = propertyContainer.findViewById(R.id.btnSaveProps)

    // Specific Props
    private val containerSpecificProps: LinearLayout? =
            propertyContainer.findViewById(R.id.containerSpecificProps)

    // NEW: Button Specific Props - REMOVED (Handled in Definition)
    // private val containerButtonProps: View? =
    // propertyContainer.findViewById(R.id.containerButtonProps)
    ...
    private fun setupListeners() {
        ...
        etTopicName?.addTextChangedListener(
                object : android.text.TextWatcher {
                    override fun beforeTextChanged(
                            s: CharSequence?,
                            start: Int,
        ...
        etPropGenericPayload?.addTextChangedListener(
                object : android.text.TextWatcher {
                    override fun afterTextChanged(s: android.text.Editable?) {
    ...
    /**
     * V18.5: updates the internal data reference from the ViewModel This ensures that when we save
     * property changes, we rely on the LATEST geometry (x, y, w, h) from the ViewModel, rather than
     * stale data from when the sheet opened.
     */
    fun updateCurrentData(newData: ComponentData) {
        // Only update if IDs match to avoid race conditions
    ...
    private fun saveCurrentProps() {
        if (selectedViewId != View.NO_ID && currentData != null) {
            try {
                ...
                val hInput = etPropHeight?.text.toString()

                // Allow empty string to just be "not saved" yet, rather than forcing 100
                if (wInput.isEmpty() || hInput.isEmpty()) return

                val density = propertyContainer.resources.displayMetrics.density

                val wDp = wInput.toFloatOrNull() ?: 100f
                val hDp = hInput.toFloatOrNull() ?: 100f
    ...
    fun showProperties(view: View, data: ComponentData, autoExpand: Boolean = true) {
        ...
        try {
            ...
            val spPresets =
                    propertyContainer.findViewById<android.widget.Spinner>(R.id.spPayloadPresets)
            if (spPresets != null) {
                ...
                spPresets.onItemSelectedListener =
    ...
    fun getLastSelectedId(): Int? =
            if (lastComponentData != null && lastViewId != View.NO_ID) lastViewId else null
